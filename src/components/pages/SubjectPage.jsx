...
export default function SubjectPage({ config, saveConfig, T, darkMode, currentUser, updateUser, subject, onBack, isEditorSession, onOpenFolder, canonicalizeGrade, canonicalizeBranch, EMOJI, SEC_EMOJI }) {
  ...
  const rawSections = Array.isArray(storedSections)
    ? storedSections
    ...
    <div className="app-shell" style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Cairo',sans-serif", direction: "rtl", paddingBottom: "30px" }}>
      <div style={{ background: T.card, backdropFilter: "blur(16px)", borderBottom: `1px solid ${T.cardBorder}`, padding: "16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          {(config.subjectIcons?.[sub] || EMOJI[sub]) && <span style={{ fontSize: "36px" }}>{config.subjectIcons?.[sub] || EMOJI[sub]}</span>}
          <div>
        ...
        <div onClick={() => setShowLessons(prev => !prev)} style={{ marginTop: "14px", background: T.sectionBg, borderRadius: "12px", padding: "12px", cursor: "pointer", border: `1px solid ${T.cardBorder}`, width: "100%", maxWidth: "100%", boxSizing: "border-box", marginLeft: 0, marginRight: 0 }}>
      ...
      <div style={{ paddingTop: "16px", paddingBottom: "16px", paddingLeft: "16px", paddingRight: "16px", display: "flex", flexDirection: "column", gap: "10px", width: "100%", boxSizing: "border-box", overflowX: "visible" }}>
        {sections.length === 0 ? <div style={{ textAlign: "center", padding: "40px", background: T.card, borderRadius: "16px", border: `1px solid ${T.cardBorder}`, width: "100%", maxWidth: "100%", boxSizing: "border-box", marginLeft: 0, marginRight: 0 }}><div style={{ fontSize: "48px" }}>📭</div><p style={{ color: T.subtext }}>لا توجد أقسام مضافة لهذه المادة</p><p style={{ color: T.subtext, fontSize: "12px" }}>يمكنك إضافة أقسام من لوحة الإدارة → أقسام المادة</p></div> : sections.map((sec) => {
          ...
          return <button key={sec} onClick={handleOpenFolder} style={{ background: T.card, border: `1px solid ${T.cardBorder}`, borderRadius: "16px", padding: "10px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: "54px", gap: "12px", cursor: "pointer", backdropFilter: "blur(10px)", textAlign: "right", width: "100%", maxWidth: "100%", boxSizing: "border-box", marginLeft: 0, marginRight: 0 }}><span style={{ fontSize: "15px", fontWeight: "600", color: T.text, flex: 1, minWidth: 0, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{SEC_EMOJI?.[sec] && <span style={{ marginLeft: "6px" }}>{SEC_EMOJI[sec]}</span>}{sec}</span><span style={{ color: T.subtext, fontSize: "16px" }}>‹</span></button>;
        })}
      </div>
    </div>
  );
