...
export default function AdminPanel({ config, saveConfig, T, darkMode, editorRole, editorPermissions, onBack, getSubjectsByGradeBranch, normalizeFoundKey, validateRequiredFields, normalizeDriveFolderInput, extractDriveFolderId, cloudflareWorkerBaseUrl, addFolderToTree, dissolveFolderInTree, getSubjectNames, canonicalizeGrade, canonicalizeBranch, normalizeFolderKey, getFolderKeyCandidates, fbGet, fbSet, fbAdd, fbDelete, fbQuery, getNews, addNewsItem, deleteNewsItem, normalizeNewsItem, formatNewsDate, sendLocalNotification }) {
  ...
  if (section !== "main" && isSectionAllowed(section)) {
    ...
    if (section === "sections") return <AdminSections config={config} saveConfig={saveConfig} T={T} onBack={() => { setActiveSubSection(null); setSection("main"); }} getSubjectNames={getSubjectNames} role={role} />;
    if (section === "folders") return <AdminFolders config={config} saveConfig={saveConfig} T={T} onBack={() => { setActiveSubSection(null); setSection("main"); }} canonicalizeGrade={canonicalizeGrade} canonicalizeBranch={canonicalizeBranch} normalizeFolderKey={normalizeFolderKey} getFolderKeyCandidates={getFolderKeyCandidates} getSubjectsByGradeBranch={getSubjectsByGradeBranch} fbGet={fbGet} fbSet={fbSet} fbDelete={fbDelete} fbQuery={fbQuery} extractDriveFolderId={extractDriveFolderId} cloudflareWorkerBaseUrl={cloudflareWorkerBaseUrl} dissolveFolderInTree={dissolveFolderInTree} />;
